...
function sanitizeOrderableMenu(categories: Category[]): Category[] {
  ...
  return categories.map((category) => ({
    ...
    items: (category.items || []).filter((item) => {
      // Never expose items that backend marks as unavailable: they can appear
      // in UI but fail at order submission.
    ...
    try {
      ...
      if (cached) {
        ...
        if (age < MENU_CACHE_TTL) {
          ...
          return sanitizeOrderableMenu(data);
        }
      }
    } catch {
...
async function fetchMenuDirect(): Promise<Category[]> {
    ...
    items: (category.items || [])
      ...
      .map((item: any) => {
        ...
        const explicitUnavailable = (item.ingredients || [])
          .filter(
            (assoc: any) =>
              !assoc.isPrimary && assoc.ingredient && !assoc.ingredient.inStock,
          )
          .map((assoc: any) => ({
  ...
  // Salva in cache
  const sanitized = sanitizeOrderableMenu(filtered as Category[]);
  saveMenuCache(sanitized);
...
export const getMenuISR = async (): Promise<Category[]> => {
  ...
  return sanitizeOrderableMenu(menu as Category[]);
};
