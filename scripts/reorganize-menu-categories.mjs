...
const supabaseKey =
  ...
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJpZWZ3enJwcmpxdXNqeW5xd3VzIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2NDEwODMxOCwiZXhwIjoyMDc5Njg0MzE4fQ.088dP_1Bxz-LHMcEDwn0Q3k6LsYUhLfUc31DT7QUXFM";
...
async function reorganizeMenu() {
  ...
  for (const [itemId, categoryName] of Object.entries(itemMappings)) {
    ...
    const { error } = await supabase.from("MenuItem").update({ categoryId }).eq("id", itemId);
  ...
  for (const update of categoryUpdates) {
    ...
    const { error } = await supabase.from("Category").update(updateData).eq("name", update.name);

    if (error) {
      console.error(`❌ Errore aggiornando ${update.name}:`, error.message);
    } else {
      console.log(
        `✅ Aggiornata categoria: ${update.name} -> sortOrder: ${update.sortOrder}${update.newName ? `, nome: ${update.newName}` : ""}`
      );
    }
  }
