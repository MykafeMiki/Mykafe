...
Deno.serve(async (req) => {
  ...
  try {
    // Scrittura massiva delle traduzioni: solo admin.
    ...
    for (const item of menuItemTranslations) {
      ...
      const { error } = await supabase.from("MenuItem").update(updateData).eq("name", item.name);
    ...
    for (const mod of modifierTranslations) {
      ...
      const { error } = await supabase.from("Modifier").update(updateData).eq("name", mod.name);
