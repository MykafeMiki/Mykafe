...
async function getOutOfStockIds(supabase: ReturnType<typeof createClient>): Promise<Set<string>> {
  ...
  const { data: ingredients } = await supabase.from("Ingredient").select("id").eq("inStock", false);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "GET" && subPath.length === 0) {
      ...
      const substituteMap: Record<string, unknown> =
            ...
            (item: {
              ...
              if (!item.available) {
                ...
                const hasPrimaryOutOfStockWithSubstitute = item.ingredients?.some(
                  (ing: { isPrimary: boolean; ingredient: { inStock: boolean; id: string } }) => {
          ...
          .map(
            (item: {
              ...
              const explicitUnavailable = (item.ingredients || [])
                .filter(
                  (assoc) => !assoc.isPrimary && assoc.ingredient && !assoc.ingredient.inStock
                )
                .map((assoc) => ({
    ...
    if (req.method === "GET" && subPath[0] === "items" && subPath[1] && !subPath[2]) {
      ...
      const filteredItem = {
        ...
        modifierGroups: item.modifierGroups?.map(
          (group: { modifiers: { available: boolean }[] }) => ({
            ...group,
    ...
    if (req.method === "POST" && subPath[0] === "items") {
      ...
      if (modifierGroups && modifierGroups.length > 0) {
        for (const group of modifierGroups) {
          ...
          if (group.modifiers && group.modifiers.length > 0) {
            const modifiersToInsert = group.modifiers.map(
              (mod: { name: string; price?: number; ingredientId?: string }) => ({
                name: mod.name,
            ...
            const { error: modError } = await supabase.from("Modifier").insert(modifiersToInsert);
      ...
      if (ingredientIds && ingredientIds.length > 0) {
        const ingredientAssociations = ingredientIds.map(
          (ing: { id: string; isPrimary?: boolean }) => ({
            menuItemId: item.id,
            ingredientId: ing.id,
    ...
    if (
      ...
      if (modifiers && modifiers.length > 0) {
        const modifiersToInsert = modifiers.map(
          (mod: { name: string; price?: number; ingredientId?: string }) => ({
            name: mod.name,
            price: mod.price ? Math.round(mod.price * 100) : 0,
            modifierGroupId: modGroup.id,
    ...
    if (req.method === "DELETE" && subPath[0] === "modifiers" && subPath[1]) {
      ...
      const { error } = await supabase.from("Modifier").delete().eq("id", modifierId);
    ...
    if (req.method === "DELETE" && subPath[0] === "modifier-groups" && subPath[1]) {
      ...
      const { error } = await supabase.from("ModifierGroup").delete().eq("id", groupId);
    ...
    if (
      ...
      await supabase.from("MenuItemIngredient").delete().eq("menuItemId", menuItemId);
      ...
      if (ingredients && ingredients.length > 0) {
        ...
        const { error } = await supabase.from("MenuItemIngredient").insert(associations);
