  ...
  try {
    ...
    if (req.method === "POST" && subPath.length === 0) {
      ...
      if (!validation.success) {
        return new Response(
          JSON.stringify({ error: "Validation error", details: validation.error }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          }
      ...
      if (table?.isCounter && !customerName) {
        return new Response(
          JSON.stringify({ error: "Customer name required for counter orders" }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          }
        ...
        if (orderTable) {
          ...
          if (belongsToSession) {
            ...
            return new Response(
              JSON.stringify({
                error:
                  "This table is part of an active group session. Please use the session to order.",
      ...
      const menuItemIds = [
        ...new Set(items.map((item: { menuItemId: string }) => item.menuItemId)),
      ];
      ...
      const allModifierIds = items.flatMap(
        (item: { modifierIds?: string[] }) => item.modifierIds || []
      );
      ...
      if (orderType !== "TAKEAWAY" && tableId) {
        await supabase.from("Table").update({ status: "OCCUPIED" }).eq("id", tableId);
      }
